...
public class CategoryDAO {
    ...
    public List<CategoryExpensesDTO> referenceOverallExpensesByCategory(int userId) {
        ...
        StringBuilder sql = new StringBuilder("SELECT c.name, COALESCE(SUM(t.amount),0) + COALESCE(SUM(p.amount),0) + COALESCE(SUM(b.amount),0) AS expense FROM categories c " +
